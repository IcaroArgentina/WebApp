...
const Consultas = ({ loggedUser }) => {
  ...
  const hidden = consultas?.find((msg) => msg.hidden === false);
  ...
  const markAsRead = async (id, currentConsulta, msg) => {
    ...
    const ref = doc(db, `Usuarios/${currentUser.uid}/Consultas`, id);
  ...
  return (
    <ConsultasMainContainer mobile={mobile}>
      ...
      <MessageContainer mobile={mobile}>
        {pending || !consultas ? (
          <Loader />
        ) : consultas.length === 0 || hidden === undefined ? (
          <NoMessages />
        ) : (
          <>
            {consultas.map((item, index) => {
              ...
              return (
                <>
                  {!item?.hidden && (
                    <Message
                      isRead={lastMsg.read ? true : false}

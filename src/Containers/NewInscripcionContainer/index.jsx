...
const NewInscripcionContainer = ({
  ...
  const comisionesFromCoursesOptions = comisionesOptions.filter((elem) => {
    return elem.nombreCurso === newData.inscripcionCurso;
  });
  ...
  function getCategoryID() {
    if (newData?.categoria?.length > 3) {
      const selectedCategoria = userOptions.filter(
  ...
  function handleSubmit(e) {
    ...
    if (newData) {
      const ref = collection(db, `Usuarios/${newData.user}/Inscripciones`);
      const createdInscripcion = {
        esEstudiante: newData.esEstudiante,
        estadoInscripcion: newData.estadoInscripcion,
        inscripcionComision: newData.inscripcionComision,
        inscripcionCurso: newData.inscripcionCurso,
      };
...
export default NewInscripcionContainer;
